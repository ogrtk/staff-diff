...
### 同期アクション別出力制御
各同期アクション（新規追加、更新、削除、変更なし）の出力を個別に制御可能：

#### sync_action_labels設定での制御（推奨）
```json
{
  "sync_rules": {
    "sync_action_labels": {
...
### 設定方法
現在は`sync_action_labels`設定でのみ制御可能です

### 出力制御の効果
- `enabled: false`に設定されたアクションは出力CSVから除外
- レポートには全処理件数と実際の出力件数の両方を表示
- 除外されたアクションには「(出力除外)」の表記を追加
