...
各同期アクション（新規追加、更新、削除、変更なし）の出力を個別に制御できます。

#### 設定方法

`sync_action_labels`設定の各アクションに`enabled`プロパティを追加して制御します：
...
- `enabled: false`: 該当アクションを出力から除外
- `enabled: true` (または未設定): 該当アクションを出力に含める

#### 実行結果の表示

出力フィルタリングが適用された場合、レポートには詳細情報が表示されます：
...
変更なし (9): 1 件
同期処理総件数: 8001 件
実際の出力件数: 1 件 (フィルタリング適用済み)
```

- `(出力除外)`: 該当アクションが出力から除外されている
- **同期処理総件数**: システム内で処理された全レコード数
- **実際の出力件数**: 出力ファイルに含まれるレコード数
