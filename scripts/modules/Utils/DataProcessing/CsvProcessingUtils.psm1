...
# CSV・SQLカラム名マッピング関数
function Get-CsvToSqlColumnMapping {
    param(
    ...
    try {
        ...
        # CSVカラム名 → SQLカラム名のマッピングを作成（エスケープは DatabaseUtils で処理）
        foreach ($column in $tableConfig.columns) {
            if ($column.csv_include -eq $true) {
                $csvName = $column.name  # CSV側は元のカラム名
                $sqlName = $column.name  # SQL側も元のカラム名（エスケープは利用側で処理）
                $mapping[$csvName] = $sqlName
            }
        }
        ...
        Write-SystemLog "CSV・SQLカラムマッピングの取得に失敗しました: $($_.Exception.Message)" -Level "Error"
        throw
    }
}

# SQL識別子からCSVカラム名への逆引きマッピング
function Get-SqlToCsvColumnMapping {
    param(
        [Parameter(Mandatory = $true)]
        [string]$TableName
    )
    ...
    try {
        $csvToSqlMapping = Get-CsvToSqlColumnMapping -TableName $TableName
        $sqlToCsvMapping = @{}
        
        # キーと値を逆転させる
        foreach ($key in $csvToSqlMapping.Keys) {
            $sqlToCsvMapping[$csvToSqlMapping[$key]] = $key
        }
        
        return $sqlToCsvMapping
    }
    catch {
        Write-SystemLog "SQL・CSVカラムマッピングの取得に失敗しました: $($_.Exception.Message)" -Level "Error"
        throw
    }
}
...
Export-ModuleMember -Function @(
    'Get-CsvToSqlColumnMapping',
    'Get-SqlToCsvColumnMapping',
    'Get-CsvFormatConfig',
